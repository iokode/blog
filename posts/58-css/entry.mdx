...
Patterns like Atomic Design a bit too complex for my needs. I usually don't need to split a component into a lot
of different parts. In my case, my organizational pattern is based on three types of stylesheet files: tokens,
components, and layout.

### Tokens
Tokens are some decisions that affect all components and layouts, like the colors, fonts, spacing, sizes, etc. 
...
should be placed in any part of the page. The pages have their own layout that positions their elements and components.
...
A `layout` directory doesn't exist here because I set the layout with the page that it belongs. If it is not possible,
...
In the layout stylesheets, I usually use tags and ids to select the layout elements.
...
But then, I found a [guide from @mdo, the creator of Bootstrap with another proposal for classifying and ordering CSS
properties](https://codeguide.co/#declaration-order). For the following projects, I'll follow it because I consider it well-argued, so I avoid reinventing

This is the @mdo's declaration order and classification:
- Positioning
- Box model
- Typographic
- Visual
...
## Reset
I always use the [Bootstrap Reboot](https://getbootstrap.com/docs/4.0/content/reboot/) library to reset the browser
styles.
...
to the browser window size. They're especially useful dynamic sizing that needs to scale proportionally with the
viewport, avoiding usage of media queries for this purpose.

I reserve physical units (`cm`, `mm`, `in`) for print stylesheets.

In resume:
- `rem` and `em` for spacing, typography, and component sizes.
- `px` for pixel-perfect details like borders.
- `%`, `vw`, `vh` for layout and responsive elements that need to adapt to viewport.
- `cm`, `mm`, `in` for print media where physical measurements matter.

## Responsive design
When I need to make a responsive design, the first strategy that I follow is to use the relative-to-viewport units, so
...
The breakpoints that I usually use are:
- 768px (tablet)
- 992px (desktop)
...
Many CSS pre-processors allow you to define variables. In the past, I used to use them to define tokens from the
...
The most important features of pre-processors now have native alternatives in CSS, even when they are implemented
differently.

An example is the use of Sass's macros to generate values for a property based on dark or light themes, but with
native CSS you can achieve the same result with the custom-properties that can be changed in runtime.

Additionally, integration can be challenging with certain frameworks that either don't support pre-processors at all
or only work with specific ones. For example, Astro supports Sass without problems, but Blazor doesn't.

Note that that doesn't mean that you couldn't use a pre-processor to generate a native CSS file in the frameworks,
