...
In the layout stylesheets, I usually use tags and ids to select the elements. There is not a collision risk with other
pages because the layout stylesheets are included only on the pages that they belong.
...
But then, I found [a guide](https://codeguide.co/#declaration-order) from @mdo, the creator of Bootstrap, with another proposal for classifying and ordering CSS
...
There is not a mobile breakpoint because I treat any viewport smaller than the `tablet` breakpoint as mobile.
...
Note that that doesn't mean that you couldn't use a preprocessor to generate a native CSS file in the frameworks,
